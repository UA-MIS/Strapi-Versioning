...
module.exports = {
  ...
  /**
   * Returns versions of an entity of a content type by id
   */
  async fetchVersions(ctx) {
    console.log('ContentManager controller: ', ctx.params);
    const contentManagerService =
      strapi.plugins['content-manager'].services.contentmanager;

    const versions = await contentManagerService.fetchVersions(ctx.params);
